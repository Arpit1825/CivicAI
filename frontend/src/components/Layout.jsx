  ...
  return (
   <div className="h-screen overflow-hidden bg-slate-50 flex">
      ...
      <aside className="hidden lg:flex lg:flex-shrink-0 lg:w-64 bg-slate-900 border-r border-slate-800 flex-col text-slate-300">
        ...
        <div className="h-16 flex items-center px-6 border-b border-slate-800">
          <Link to="/dashboard" className="flex items-center space-x-2.5">
            ...
            <span className="text-xl font-bold tracking-tight text-white">Civic<span className="text-blue-500">AI</span></span>
          </Link>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-3">
                  <ActiveIcon className={`h-4.5 w-4.5 ${isItemActive ? 'text-white' : 'text-slate-400 group-hover:text-white'}`} />
                  <span>{item.name}</span>
                </div>
                {item.highlight && !isItemActive && (
                  <span className="bg-blue-500/20 text-blue-400 text-xs px-2 py-0.5 rounded-full font-semibold animate-pulse-slow">
                    New
                  </span>
                )}
              </Link>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3 overflow-hidden">
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-xs text-slate-500 truncate">{currentUser.role === 'admin' ? 'Gov Official' : 'Citizen'}</p>
              </div>
            </div>
            <button 
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 lg:px-8 z-40 sticky top-0">
          ...
          <div className="flex items-center space-x-4">
            ...
            {!isAdmin && (
              <div className="hidden sm:flex items-center space-x-3 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-lg px-3 py-1.5">
                <Award className="h-5 w-5 text-blue-600" />
                <div>
                  <div className="flex items-center space-x-1.5">
                    <span className="text-xs font-semibold text-slate-500">Lv.{score.level}</span>
                    <span className="text-xs font-bold text-blue-700">{score.title}</span>
                  </div>
                  <div className="w-24 bg-blue-200/50 rounded-full h-1 mt-0.5 overflow-hidden">
                    <div className="bg-blue-600 h-1 rounded-full" style={{ width: `${score.progress}%` }}></div>
                  </div>
                </div>
                <div className="text-right border-l border-blue-200 pl-3">
                  <span className="text-xs text-slate-400 block">Civic Score</span>
                  <span className="text-sm font-bold text-slate-800">{score.points} pts</span>
                </div>
              </div>
            )}
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="hidden md:block text-sm font-medium text-slate-700">{currentUser.name}</span>
              </button>
              ...
              {profileDropdownOpen && (
                <>
                  ...
                  ></div>
                  <div className="absolute right-0 mt-2 w-56 rounded-xl bg-white border border-slate-200 shadow-xl py-2 z-20">
                    ...
                    {!isAdmin && (
                      <div className="px-4 py-2 bg-slate-50 border-b border-slate-100 sm:hidden">
                        <div className="flex justify-between items-center text-xs mb-1">
                          <span className="font-bold text-slate-700">{score.title} (Lv.{score.level})</span>
                          <span className="text-blue-600 font-bold">{score.points} pts</span>
                        </div>
                        <div className="w-full bg-slate-200 rounded-full h-1.5 overflow-hidden">
                          <div className="bg-blue-600 h-1.5 rounded-full" style={{ width: `${score.progress}%` }}></div>
                        </div>
                      </div>
                    )}
                    ...
                    <button
                      onClick={handleLogout}
                      className="w-full text-left px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 flex items-center space-x-2 transition-colors mt-1"
                    >
                      <LogOut className="h-4 w-4" />
                      <span>Log Out</span>
                    </button>
    ...
    {children}
      ...
      {/* Mobile Drawer Menu */}
      <div className={`fixed top-0 bottom-0 left-0 w-72 bg-slate-900 text-slate-300 z-55 transform transition-transform duration-300 ease-in-out lg:hidden flex flex-col ${
        mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
      }`}>
        <div className="h-16 flex items-center justify-between px-6 border-b border-slate-800">
          <Link to="/dashboard" className="flex items-center space-x-2.5" onClick={() => setMobileMenuOpen(false)}>
            <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center shadow-md">
              <Radar className="h-4 w-4 text-white" />
            </div>
            <span className="text-lg font-bold tracking-tight text-white">Civic<span className="text-blue-500">AI</span></span>
          </Link>
          <button 
            onClick={() => setMobileMenuOpen(false)} 
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/40">
          <div className="flex items-center justify-between">
            ...
            <button 
              onClick={handleLogout} 
              className="p-2 rounded-md hover:bg-slate-800 text-slate-400 hover:text-red-400"
            >
